import torch
from pytracking.libs import optimization, operation
from videoanalyst.online_update.tensorlist import TensorList
import math


class FactorizedConvProblem(optimization.L2Problem):
    def __init__(self, training_samples: TensorList, y:TensorList, filter_reg: torch.Tensor, projection_reg, sample_weights: TensorList,
    ...
    def __call__(self, x: TensorList):
        ...
        residuals = operation.conv2d(self.training_samples, filter, mode='same').apply(self.response_activation)
...
class ConvProblem(optimization.L2Problem):
    ...
    def __call__(self, x: TensorList, mask):
        ...
        residuals = operation.conv2d(self.training_samples, x).apply(self.response_activation)
...
class RegConvProblem(optimization.L2Problem):
    def __init__(self, training_samples: TensorList, y:TensorList, filter_reg: torch.Tensor, sample_weights: TensorList, response_activation):
        self.training_samples = training_samples
        self.y = y
        self.filter_reg = filter_reg
    ...
    def __call__(self, x: TensorList, mask):
        ...
        """
        # Do convolution and compute residuals
        residuals = operation.conv2d(self.training_samples, x, padding=1).apply(self.response_activation)
        #residuals = self.response_activation(residuals)
        residuals = residuals - self.y
...
class CtrConvProblem(optimization.L2Problem):
    def __init__(self, training_samples: TensorList, y:TensorList, filter_reg: torch.Tensor, sample_weights: TensorList, response_activation):
        self.training_samples = training_samples
        self.y = y
        self.filter_reg = filter_reg
    ...
    def __call__(self, x: TensorList, mask):
        ...
        residuals = TensorList([torch.sigmoid(residuals[0])])
        #residuals = self.response_activation(residuals)
        residuals = residuals - self.y
