...
class Transform:
    ...
    def crop_to_output(self, image):
        if isinstance(image, torch.Tensor):
            ...
            #im = image.detach().cpu().numpy()[0].transpose(1, 2, 0)
            #avg_chans = np.mean(im, axis=(0, 1, 2))

            return F.pad(image, (pad_left, pad_right, pad_top, pad_bottom), 'replicate') #'replicate' #'constant', avg_chans
        else:
            raise NotImplementedError
...
class Blur(Transform):
    ...
    def __init__(self, sigma, output_sz = None, shift = None):
        ...
        self.filter[0] = (self.filter[0].view(1,1,-1,1) / self.filter[0].sum()).to('cuda')
        self.filter[1] = (self.filter[1].view(1,1,1,-1) / self.filter[1].sum()).to('cuda')
...
def numpy_to_torch(a: np.ndarray):
    return torch.from_numpy(a).float().permute(2, 0, 1).unsqueeze(0).to('cuda')
