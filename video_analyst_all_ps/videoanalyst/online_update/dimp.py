...
class Discriminative_classifier(BaseTracker):

    def initialize(self, image, gt_bbox, backbone, extract_classification_feat, info: dict) -> dict:
        ...
        self.initializer = clf_initializer.FilterInitializerLinear(filter_size=1, filter_norm=False,
                                                              feature_dim=512) #check cls out feature dim

        # Optimizer for the DiMP classifier
        self.optimizer = clf_optimizer.DiMPSteepestDescentGN(num_iter=5, feat_stride=8,
        ...
        self.classifier = target_clf.LinearFilter(filter_size=1, filter_initializer=self.initializer,
                                             filter_optimizer=self.optimizer, feature_extractor=self.extract_classification_feat)
    ...
    def track(self, image, pred_bbox, info: dict = None) -> dict:
        self.debug_info = {}
        ...
        if flag != 'not_found':
            if self.params.get('use_iou_net', True):
                ...
                self.pos = torch.Tensor(pred_bbox[0], pred_bbox[1])#update self.pos(center and hw) self.target_sz
                self.sz = torch.Tensor(pred_bbox[3], pred_bbox[2])
    ...
    def classify_target(self, sample_x: TensorList):
        ...
        with torch.no_grad():
            scores = self.classifier.classify(self.target_filter, sample_x)
        return scores
    ...
    def get_classification_features(self, backbone_feat):
        with torch.no_grad():
            return self.extract_classification_feat(backbone_feat)
    ...
    def generate_init_samples(self, im: torch.Tensor) -> TensorList:
        ...
        with torch.no_grad():
            init_backbone_feat = self.backbone(im_patches)
    ...
    def init_classifier(self, init_backbone_feat):
        ...
        with torch.no_grad():
            self.target_filter, _, losses = self.classifier.get_filter(x, target_boxes, num_iter=num_iter,
    ...
    def _overwrite_classifier_params(self, feature_dim):
        # Overwrite some parameters in the classifier. (These are not generally changed)
        pred_module = getattr(self.classifier.filter_optimizer, 'score_predictor', self.net.classifier.filter_optimizer)
        if self.params.get('label_threshold', None) is not None:
            self.classifier.filter_optimizer.label_threshold = self.params.label_threshold
        if self.params.get('label_shrink', None) is not None:
            self.classifier.filter_optimizer.label_shrink = self.params.label_shrink
        if self.params.get('softmax_reg', None) is not None:
            self.classifier.filter_optimizer.softmax_reg = self.params.softmax_reg
        ...
        if self.params.get('filter_init_zero', False):
            self.classifier.filter_initializer = FilterInitializerZero(self.net.classifier.filter_size, feature_dim)
    ...
    def update_classifier(self, train_x, target_box, learning_rate=None, scores=None):
        ...
        if num_iter > 0:
            ...
            with torch.no_grad():
                self.target_filter, _, losses = self.classifier.filter_optimizer(self.target_filter,
