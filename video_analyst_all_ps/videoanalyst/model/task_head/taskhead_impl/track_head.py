...
from videoanalyst.model.common_opr.common_block import conv_bn_relu, conv_bn_relu_withweight
from videoanalyst.model.module_base import ModuleBase
from videoanalyst.model.task_head.taskhead_base import TRACK_HEADS, VOS_HEADS
from videoanalyst.online_update.tensorlist import tensor_operation, TensorList
...
class DenseboxHead(ModuleBase):
    ...
    def forward(self, f, c_out, r_out, new_x_size, cls_weights, ctr_weights, reg_weights):
        # classification head
        ...
        if type(cls_weights) != type(None):
            cls_score2 = self.cls_score2_p5(cls, cls_weights)  # todo
            cls_score2 = cls_score2.permute(0, 2, 3, 1)
            cls_score2 = cls_score2.reshape(cls_score2.shape[0], -1, 1)
            #ratio_cls = 0.5
            #cls_score = torch.add(ratio_cls * cls_score, (1 - ratio_cls) * cls_score2)
        else:
            cls_score2 = None

        # center-ness score
        if f != 0 and type(ctr_weights) != type(None):
            self.ctr_score_pf.conv.weight = nn.Parameter(ctr_weights[0])
        ...
        if type(reg_weights) != type(None):
            reg_label = offsets
            ratio_r = 0.5
            offsets2 = self.bbox_offsets2_p5(bbox, reg_weights)
            reg_pred = offsets2
            offsets = torch.add(ratio_r * offsets, (1 - ratio_r) * offsets2)
        else:
            reg_label = None
            reg_pred = None
        offsets = torch.exp(self.si * offsets + self.bi) * self.total_stride
        # adjust search area
        self._hyper_params["x_size"] = new_x_size
        x_size = self._hyper_params["x_size"]
        score_size = (self._hyper_params["x_size"] -
                    127) // self._hyper_params['total_stride'] + 1 - self._hyper_params['num_conv3x3'] * 2
        ...
        self._hyper_params["score_size"] = score_size
        ...
        return [cls_score, ctr_score, bbox, cls, reg_feat, cls_score2, reg_label, reg_pred]
