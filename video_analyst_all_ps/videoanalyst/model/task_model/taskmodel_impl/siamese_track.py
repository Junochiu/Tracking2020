...
class SiamTrack(ModuleBase):
    ...
    def forward(self, *args, phase="train", f = None, new_x_size, cls_weights = None, ctr_weights = None, reg_weights = None):
        r"""
        ...
        if phase == 'train':
        ...
        elif phase == 'precisefeature':
            whole_img, = args
            f_z = self.basemodel(whole_img)
            out_list = f_z
            ...
            c_out = xcorr_depthwise(c_x, c_z_k)
            # head
            fcos_cls_score_final, fcos_ctr_score_final, fcos_bbox_final, corr_fea, reg_feature, cls2_score, reg_label, reg_pred = self.head(
                f, c_out, r_out, new_x_size, cls_weights, ctr_weights, reg_weights)
            # apply sigmoid
            fcos_cls_prob_final = torch.sigmoid(fcos_cls_score_final)
            if type(cls_weights) != type(None):
                cls_ratio = 0.8
                fcos_cls_prob_final = cls_ratio * fcos_cls_prob_final + (1 - cls_ratio) * cls2_score
            fcos_ctr_prob_final = torch.sigmoid(fcos_ctr_score_final)
            '''
            if type(cls2_score) != type(None):
                cls2_score = torch.sigmoid(cls2_score)
            '''
            ...
            # output
            out_list = fcos_score_final, fcos_bbox_final, fcos_cls_prob_final, fcos_ctr_prob_final, extra, corr_fea, reg_feature, cls2_score, reg_label, reg_pred
        else:
