...
class Tracker:
    ...
    def _track_sequence(self, tracker, seq, init_info):
        ...
        # Initialize
        image_path = ''
        for i in range(len(seq.frames[0].split('/')[:])):
            if (i != 0 and i != len(seq.frames[0].split('/')[:]) - 2):
                image_path += '/' + seq.frames[0].split('/')[i]
        image, image_show = self._read_image(image_path)
        ...
        outbox = init_info.get('init_bbox')
        image_show = image_show.copy().astype(np.uint8)
        cv.rectangle(image_show, (int(outbox[0]), int(outbox[1])),
                     (int(outbox[2] + outbox[0]), int(outbox[3] + outbox[1])),
                     (0, 255, 0), 5)
        ...
        for frame_num, frame_path in enumerate(seq.frames[1:], start=1):
            ...
            image_path = ''
            for i in range(len(frame_path.split('/')[:])):
                if (i != 0 and i != len(frame_path.split('/')[:]) - 2):
                    image_path += '/' + frame_path.split('/')[i]
            image, image_show = self._read_image(image_path)
    ...
    def _read_image(self, image_file: str):
        ...
        return cv.cvtColor(im, cv.COLOR_BGR2RGB), im
