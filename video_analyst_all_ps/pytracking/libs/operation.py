...
def conv2d(input: torch.Tensor, weight: torch.Tensor, bias: torch.Tensor = None, stride=1, padding=0, dilation=1, groups=1, mode=None):
    ...
    conv = nn.Conv2d(256,
                    ...
                    groups=groups)
    '''
    x = F.conv2d(input, weight, bias=None, stride=stride, padding=padding, dilation=dilation, groups=groups)
    #conv.weight = nn.Parameter(weight.cuda())
    #x = conv(input)
...
def conv1x1(input: torch.Tensor, weight: torch.Tensor):
    ...
    if weight is None:
        return input
    #print('type:', type(input), input.size())
