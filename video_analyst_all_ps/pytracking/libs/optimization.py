...
class ConjugateGradient(ConjugateGradientBase):
    ...
    def run(self, num_cg_iter, mask=None):
        ...
        self.f0 = self.problem(self.x, mask)
...
class GaussNewtonCG(ConjugateGradientBase):
    ...
    def run(self, num_cg_iter, num_gn_iter=None):
        ...
        #print('cg_iter', num_cg_iter, type(num_cg_iter))
        ...
        return self.losses, self.residuals, self.x
    ...
    def run_GN_iter(self, num_cg_iter):
        ...
        self.x.requires_grad_(True)
        #print('self.x', type(self.x), self.x.size())

        # Evaluate function at current estimate
        self.f0 = self.problem(self.x)
        #print('self.f0', type(self.f0), self.f0.size())
        ...
        self.g.requires_grad_(True)
        #print('self.g', type(self.g), self.g.size())

        # Get df/dx^t @ f0
        self.dfdxt_g = TensorList(torch.autograd.grad(self.f0, self.x, self.g, create_graph=True))
        #print('self.dfdxt_g', type(self.dfdxt_g), self.dfdxt_g.size())

        # Get the right hand side
        self.b = - self.dfdxt_g.detach()
        #print('self.b', type(self.b), self.b.size())
        ...
        print('during update', self.x[0][0][20][0])
